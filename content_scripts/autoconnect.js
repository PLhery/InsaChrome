...
chrome.runtime.sendMessage({method: "getInfos"}, function(response) { //on récupère les préferences utilisateur
    if(response['autoconnect'] == "true") { //Si l'utilisateur a activé la connexion automatique aux sites INSA
        ...
        if(document.getElementsByClassName('visualIconPadding').length && document.getElementsByClassName('visualIconPadding')[0].innerHTML.indexOf("S'identifier")>=0){ //Si il y a un lien "s'identifier" (cipcnet), on se connecte
            let element = document.getElementsByClassName('visualIconPadding')[0]; //L'élement sur lequel on va afficher "connexion..", c'est le fameux lien
        ...
        if(document.getElementsByClassName('logininfo').length && (document.getElementsByClassName('logininfo')[0].innerHTML.indexOf("Non connecté.")>=0 || document.getElementsByClassName('logininfo')[0].innerHTML.indexOf("Vous êtes connecté anonymement")>=0)){ //Si il y a un lien "Non connecté." (moodle), on se connecte
            let element = document.getElementsByClassName('logininfo')[0]; //L'élement sur lequel on va afficher "connexion..", c'est le fameux lien
        ...
        if(document.getElementById('portalCASLoginLink') && document.getElementById('portalCASLoginLink').innerHTML.indexOf("Identifiez-vous")>=0){ //Si il y a un lien-bouton "Identifiez-vous" (planète), on se connecte
            let element = document.getElementById('portalCASLoginLink'); //L'élement sur lequel on va afficher "connexion..", c'est le fameux lien-bouton
        ...
        if(document.getElementsByClassName('connect').length && document.getElementsByClassName('connect')[0].value.indexOf("Se connecter")>=0){ //Si il y a un bouton "Se connecter" (zimbra), on se connecte
            let element = document.getElementsByClassName('connect')[0]; //L'élement sur lequel on va afficher "connexion..", c'est le fameux bouton
        ...
        if(document.getElementsByClassName('btn-submit').length && document.getElementsByClassName('btn-submit')[0].value.indexOf("SE CONNECTER")>=0){ //Si il y a un bouton "SE CONNECTER" (CAS), on se connecte
            let element = document.getElementsByClassName('btn-submit')[0]; //L'élement sur lequel on va afficher "connexion..", c'est le fameux bouton
        ...
        function connexionLien(element, service, url) { //animation de connexion sur les liens
            ...
            let connected = false; //On est pas connecté
            chrome.runtime.sendMessage({method: "connect"}); //On lance la connexion


            element.style.fontWeight = "bold"; //On met son contenu en gras.

            let texte = "Connexion.."; //On prend le texte à afficher
            let couleursLettres = ["#e91e63","#00bcd4", "#8bc34a","#ffc107","#009688","#ff9800"]; //Et un gros tas de couleurs
            let i=0;
            ...
            function boucle() { //Cette fonction sera executée toutes les 80 ms
                ...
                let couleur = couleursLettres[Math.floor(Math.random() * couleursLettres.length)]; //On prend une couleur au hasard
                const connectHtml = texte.substring(0, i%11) + "<span style='color:"+couleur+"'>" + texte.charAt(i%11) + "</span>" + texte.substring(i%11+1, texte.length); //On entoure la ième lettre de cette couleur
        ...
        function connexionBouton(element, service, url) {
            ...
            function boucle() { //Cette fonction sera executée toutes les 300 ms
                ...
                const texte="Connexion"+new Array(i%4+1).join('.')+new Array(4-i%4).join(' '); //On prend le texte à afficher, avec i modulo 4 (0,1,2 ou 3) petits points (et des espaces pour garder une taille fixe)
