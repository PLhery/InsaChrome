		...
		let timeout; //On déclare la letiable timeout, contenant le dernier timeout lancé à annuler avec clearTimeout.
			...
			for (key in changes) {
				...
				if(key=="infos" && changes["infos"].newValue) { //Si on recoit les infos
					...
					if(changes[key].newValue[6] && !isNaN(changes[key].newValue[6]) && $("#afficherPokemon2").length>0) //Si on a le rang
						document.getElementById('photo').src= "https://pokeapi.co/media/img/"+changes[key].newValue[6]+".png"; //On affiche le pokemon
					else
						document.getElementById('photo').src= changes[key].newValue[3];
					...
					$("body").css("margin","5px").css("background","#FEFEFE"); //On lance l'animation
					$("#ok").slideUp();
					$("#services").slideUp();
					$("#wait").slideUp();
...
function circularAnimation() { //adapté de http://jsfiddle.net/F9pLC/
		let $wait = $("#wait");
		let width = $wait.width(),
			height = $wait.height(); //dimensions de la popup
		let diag = Math.ceil(Math.sqrt(width * width + height * height)); //On en déduit la diagonale/Le rayon max du cercle
		let pageX = width/2,
		...
		$('<div class="circle">').appendTo("#wait").css({ //On rajoute un cercle de 20*20/de rayon 10px au centre
			...
			duration: 700,
				...
				if(!$wait.is(":hidden")) //si le wait n'est pas caché on recommence
					setTimeout(circularAnimation, 0);
			}
		});
}
circularAnimation();